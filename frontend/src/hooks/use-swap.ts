...
export function useSwap() {
  ...
  const { data: quoteData, isLoading: isQuoteLoading, error: quoteErrorData, refetch: refetchQuote } = useQuery({
    queryKey: ['swap-quote', swapParams.fromToken, swapParams.toToken, swapParams.fromAmount, chainId, address],
    queryFn: async () => {
      if (!address || !chainId || !swapParams.fromAmount || parseFloat(swapParams.fromAmount) <= 0) {
        return null
      }

      const fromTokenInfo = getTokenInfo(swapParams.fromToken, chainId)
      const toTokenInfo = getTokenInfo(swapParams.toToken, chainId)

      if (!fromTokenInfo || !toTokenInfo) {
        throw new Error('Invalid token selection')
      }

      const fromAmountWei = swapService.toWei(swapParams.fromAmount, fromTokenInfo.decimals)

      const quoteResponse = await swapService.getQuote({
        fromChainId: chainId,
        toChainId: chainId, // Same chain for now
    ...
    enabled: !!address && !!chainId && !!swapParams.fromAmount && parseFloat(swapParams.fromAmount) > 0,
  ...
  }, [address, chainId, swapParams.fromAmount, quote, isQuoteLoading, quoteError, isExecuting])

  const executeSwap = useCallback(async () => {
    if (!address || !chainId || !quote || !swapParams.fromAmount) {
    ...
    try {
      ...
      const { order, verifyingContract, extension, quoteId, secretHashes, domain, types } = orderResponse.data
  ...
  }, [address, chainId, quote, swapParams.fromToken, swapParams.toToken, swapParams.fromAmount])
