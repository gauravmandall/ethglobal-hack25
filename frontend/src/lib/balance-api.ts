...
export class BalanceApiService {
  ...
  async getWalletBalances(chainId: number, walletAddress: string): Promise<BalanceResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || errorData.error || 'Failed to fetch balance');
      }
  ...
  async getETHBalance(chainId: number, walletAddress: string): Promise<string> {
    try {
      console.log(`Getting ETH balance for ${walletAddress} on chain ${chainId}`);
      ...
      if (ethBalanceWei && ethBalanceWei !== '0') {
        ...
        console.log(`Converted balance: ${balanceInWei} wei -> ${balanceInETH} ETH`);
  ...
  async getTokenBalance(chainId: number, walletAddress: string, tokenAddress: string): Promise<string> {
    try {
      const response = await this.getWalletBalances(chainId, walletAddress);
