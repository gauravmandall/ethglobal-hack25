...
export function getTokenAddress(symbol: string, chainId: number): string {
  const chainTokens =
    COMMON_TOKENS[chainId.toString() as keyof typeof COMMON_TOKENS];
  if (!chainTokens) {
    // Default to Ethereum if chain not found
    const ethTokens = COMMON_TOKENS["1"];
    return (
      ethTokens[symbol as keyof typeof ethTokens]?.address ||
...
export function getTokenDisplayName(symbol: string, chainId: number): string {
...
export function getSupportedTokens(chainId: number): TokenInfo[] {
  ...
  if (!chainTokens) {
    // Default to Ethereum if chain not found
    const ethTokens = COMMON_TOKENS["1"];
    return Object.values(ethTokens);
  }
